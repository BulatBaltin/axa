...
<div class="tabs_wrapper">
    <div class="tabs">
        ...
        <span class="tab"><?l('Emails')?></span>
    </div>
    <? $form->start('open_task') ?>
    <div>
        <?  $form->errors() ?>
        <div hidden>
            <?  $form->row('rulelist') ?>
...
<? include('report-content.html.php') ?>
...
<label for="addrule"><i class="fas fa-user-plus mr-2"></i><?l('Assign Rule to exclude')?></label>
        ...
        <div class="tab_item">
            ...
            <div class="mb-4 mt-3" style='display:flex; align-items: center;'>
                <h4><?l('Email recievers')?></h4>
                <div style='display:inline-block;background:#eee; border: 2px solid #ddd;padding: 0.2rem 1rem;border-radius:1.2rem;margin-left:5rem;'>
                    <a id='send-email' class="no-text-decor" href="javascript:void(0)">&#171; <?l('Send emails') ?></a>
                </div>            
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div id="add-user" class="form-group custom_select_wrap mt-3">
                        <label for="adduser"><i class="fas fa-user-plus mr-2"></i><?l('Assign Employee to Email subscription')?></label>
                        <? $form->widget('adduser') ?>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#send-email").click(function(){
            ...
            dmDialog({
                title: 'Quickvoice',
                text: '<?l("Send open task report emails")?>',
                ok: function() {
                    ShowProcess();

                    $.ajax({ method: "POST", dataType: 'json', data: form_data,
                        cache: false, processData: false, contentType: false,
                        url: "<? path('qv.task.opened-email') ?>",
                        success: function (result) {
                            HideProcess();
        ...
        $("#update-3").click(function(){
            ...
            $.ajax({ method: "POST", dataType: 'json', data: form_data,
                cache: false, processData: false, contentType: false,
                url: "<? path('qv.client.activate', ['id' => $object['id'] ]) ?>",
                success: function (result) {
        ...
        $("#update-4").click(function(){
            ...
            $.ajax({ method: "POST", dataType: 'json', data: form_data,
                cache: false, processData: false, contentType: false,
                url: "<? path('qv.client.visibility', ['id' => $object['id'] ]) ?>",
                success: function (result) {
                    $('#client_id').val(result.client_id);
