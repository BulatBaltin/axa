...
class FILE_SYSTEM {
    ...
    public static function excludeFile($file, $ext = null) {
        if($ext) {
            $fext=substr($file,strlen($ext)*-1);
            return $fext == $ext;
        }
        return false;
    }
    public static function getDirFiles($dir, $ext='php', $exclude = null)
    {
        $opened_dir = opendir($dir);
        $files = [];
        while ($element=readdir($opened_dir)){
            $fext=substr($element,strlen($ext)*-1);
            if(($element!='.') && ($element!='..') && ($fext==$ext)){
                if(self::excludeFile($element, $exclude)) continue;
                $files[] = $dir.$element;
            }
        }
        closedir($opened_dir);
