...
class APP
{
    ...
    public static function Duplex($value = BUNDLE) {
        return BUNDLE;
    }
    public static function Run($app_name="frontend")
    {
        ...
        try
        {
            ...
            $file = BUNDLE . ROUTER::ModuleName()."/__construct.php";
            file_exists($file) and include($file);

            // Step 3.
            //контроллер для action'a
            $file = BUNDLE . ROUTER::ModuleName()."/".ROUTER::ActionName().".php";
            file_exists($file) and include($file);
        ...
        if(self::isLayout()) //self::GenerateView())
        {
            ...
            //view для action'a
            $file = BUNDLE . ROUTER::ModuleName()."/".ROUTER::ActionName().".view.php";
            file_exists($file) and include($file);
        }
    }
    ...
    {
        //конфиг фреймворка
        include_once( DLIGHT . "config/config.php");

        //ядро
        include_once( DLIGHT . "core/database.php");
        include_once( DLIGHT . "core/dlplugin.php");
        FILE_SYSTEM::includeDir( DLIGHT . "core/");
        FILE_SYSTEM::includeDir( DLIGHT . "core2/");
// dd('HERE');
        ...
        //библиотеки
        FILE_SYSTEM::includeDir( DLIGHT . "lib/");
        require_once( DLIGHT . "lib/phpthumb/ThumbLib.inc.php");
        ...
        include_once( DLIGHT . "autoload.php");

        // Azure service
        require_once( ROOT . "vendor/autoload.php" );

        //библиотеки проекта
        // FILE_SYSTEM::includeDir($doc_root."lib/");
        
        
        // //конфиг приложения
        // FILE_SYSTEM::includeDir($doc_root."app/".self::Name()."/config/");
        
        // //модель приложения
        // FILE_SYSTEM::includeDir($doc_root."app/".self::Name()."/model/");
...
public static function checkForBaseRedirects(){
   ...
   public static function setControllerFileName()
   {
    ...
    public static function addAPPConfig($app_config)
    {
        foreach($app_config as $key=>$value)
            self::$_app_config[$key]=$value;
    }
    
    public static function addAPPConst($const_params)
    {
